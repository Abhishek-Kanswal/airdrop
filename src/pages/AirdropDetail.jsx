...
import IntroductionDiv from "../components/common/IntroductionDiv";
...
const ProjectDetail = () => {
  ...
  const airdrops = useSelector((state) => state.airdrop.value);
  ...
  const { id } = useParams();
  const project = airdrops.find((airdrop) => airdrop.name === id);
    ...
    <>
      ...
      <div className="flex w-full h-full min-h-screen bg-background items-center justify-center">
        <div className="flex flex-col items-center w-[80%] min-h-screen bg-toggleBg m-12 rounded-lg">
          <IntroductionDiv project={project} />
          ...
          <div className="w-[95%] flex-grow m-8 flex flex-col rounded-lg min-h-[300px]">
            <div className="w-full px-6 py-4">
              <h2 className="text-secondaryText text-2xl font-semibold">
                Instructions for completing tasks and activities for{" "}
                {project.name}
              </h2>
              <p className="text-secondaryText text-base mt-2">
                Complete the tasks to qualify for potential {project.name}{" "}
                airdrops and rewards. Follow step-by-step instructions and track
                task updates and statuses to become one of the potential
                recipients of the {project.name} Airdrop.
              </p>
            </div>
            ...
            <div className="flex flex-row flex-grow h-full m-5">
              ...
              <div className="h-full w-[75%] flex-grow flex items-center justify-center text-secondaryText-400 font-medium">
                <p className="text-center text-lg">
                  Follow the instructions provided to complete the tasks and
                  qualify for rewards.
                </p>
              </div>
            </div>
